...
| [0005](0005-act-as-the-oidc-client-with-server-side-sessions.md) | Act as the OIDC client, with server-side sessions | Accepted | The BFF pattern per RFC 10017, chosen because `EventSource`, downloads, and `<img>` cannot send headers. Requires writing the session store and a documented `rsa` advisory exception. |
