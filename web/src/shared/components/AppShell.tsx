...
export function AppShell({
  ...
  return (
    <div className="grid h-dvh grid-cols-[13rem_1fr] grid-rows-[1fr_auto]">
      <Sidebar />
      {/* `min-h-0` so the `1fr` row may be shorter than its content, which
          a grid item's default `min-height: auto` would refuse. A screen
          fills this with `h-full`, never `h-dvh` — that is the whole window,
          and this is the window less the status bar. */}
